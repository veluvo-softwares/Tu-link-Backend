...
export class JourneyController {
  ...
  @ApiOperation({
    ...
    description: 'Create a new journey with PENDING status. The authenticated user becomes the leader.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Get detailed information about a journey including all participants. User must be a participant.'
  })
  @ApiResponse({ status: 200, description: 'Journey details retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Not a participant of this journey' })
  ...
  @ApiOperation({
    ...
    description: 'Update journey details. Only the leader can update. Can only update PENDING journeys.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Cancel a journey. Only the leader can cancel. Cannot cancel active journeys.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Start the journey, changing status from PENDING to ACTIVE. All ACCEPTED participants become ACTIVE. Only the leader can start.'
  })
  @ApiResponse({ status: 200, description: 'Journey started successfully' })
  @ApiResponse({ status: 400, description: 'Journey already started or completed' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiOperation({
    summary: 'End journey',
    description: 'End the journey, changing status from ACTIVE to COMPLETED. Only the leader can end.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Get all participants in a journey with their status and details.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Invite a user to join the journey. Only the leader can invite. Journey must be PENDING. Creates a notification for the invited user.'
  })
  @ApiResponse({ status: 201, description: 'Invitation sent successfully' })
  @ApiResponse({ status: 400, description: 'Can only invite to pending journeys OR User already invited/participating OR Cannot invite yourself' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Only leader can invite participants' })
  @ApiResponse({ status: 404, description: 'Journey not found OR Invited user not found' })
  async invite(
    @Param('id') id: string,
    @CurrentUser('uid') userId: string,
  ...
  @ApiOperation({
    ...
    description: 'Accept an invitation to join a journey. Changes participant status from INVITED to ACCEPTED. Sets joinedAt timestamp.'
  })
  @ApiResponse({ status: 200, description: 'Invitation accepted successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Journey not found OR Invitation not found' })
  ...
  @ApiOperation({
    ...
    description: 'Decline an invitation to join a journey. Changes participant status from INVITED to DECLINED.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Leave an active journey. Changes participant status to LEFT. Cannot leave if you are the leader.'
  })
  @ApiResponse({ status: 200, description: 'Left journey successfully' })
  @ApiResponse({ status: 400, description: 'Cannot leave - journey requirements not met' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Journey not found OR Not a participant' })
